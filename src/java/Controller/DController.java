    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (act.equals("viewcomment")) {
         ...
         if (act.equals("comment")) {
            String op = request.getParameter("get");
            Gson json = new Gson();
            PostModelAdmin pos = new PostModelAdmin();
            ...
            String s=String.valueOf(c);
             PostAdmin p = pos.getPost(s);
             String OwnerPost = p.getEmail();
             String postID = p.getPostID();
             String postTitle = p.getTitle();
            String toMail = cmt.getEmail();
            sen.sendEmail("BTECLAND | We have received comments from you", "We will contact you as soon as possible. Thank you for your interest in our products.", toMail);
            sen.sendEmail("BTECLAND | Notice of customer comments on the Post have ID: " + postID,
                           "Comments from customer on the post have the title: " + postTitle+"; "
                                   + "Customer Name: " + cmt.getName()+"; "
                                           + "Customer Email: "  + cmt.getEmail()+ "; "
                                                   + "Customer Phone number: " +cmt.getPhone()+"; "
                                                           + "Comment content: " +cmt.getContent(),
                           OwnerPost);
