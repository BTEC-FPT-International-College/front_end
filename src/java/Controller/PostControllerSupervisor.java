 ...
 * @author MarkTran
 */
@WebServlet(name = "PostControllerSupervisor", urlPatterns = {"/PostControllerSupervisor"})
public class PostControllerSupervisor extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String act = request.getParameter("ac");
            ...
            PostModelAdmin am = new PostModelAdmin();
             Gson json = new Gson();
             ArrayList<PostAdmin> a = am.getListPostByCate(op);
        ...
        if (act.equals("search")) {
            ...
            PostModelAdmin am = new PostModelAdmin();
            ArrayList<PostAdmin> a = am.searchDateBycate(sup.getStart(), sup.getEnd(), sup.getCategoryid());
            String listTrainee = json.toJson(a);
            response.setContentType("text/html");
            response.getWriter().write(listTrainee);
        }
        ...
        if (act.equals("search1")) {
            ...
            Search_Recharge sup = json.fromJson(op, Search_Recharge.class);
            PostModelAdmin am = new PostModelAdmin();
            ArrayList<PostAdmin> a = am.searchDate2ByCate(sup.getStart(), sup.getEnd(), sup.getCategoryid());
            String listTrainee = json.toJson(a);
            response.setContentType("text/html");
            response.getWriter().write(listTrainee);
        }
