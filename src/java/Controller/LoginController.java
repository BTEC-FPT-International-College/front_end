...
//                    dispatcher.forward(request, response);
                        GetSup g = new GetSup();
                        Profile infor = g.getCateBySub(userID);
                        String CateID = infor.getCateid();// getCategoryID()
                        response.sendRedirect("Supervisor/index.jsp?id="+CateID+"&userid="+userID); // sua duong dan
                    }
                    if (RoleID.equals("1")) {
//                    request.getRequestDispatcher("pages/index.jsp").forward(request, response);
//                    RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/MainControllers?ac=view");
