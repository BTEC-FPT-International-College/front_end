				...
				<div class="aa-latest-properties-content">
					...
					<form method="POST" id="myform">
						<select onChange="showList();" name="myselect">
							...
							<option value="2"
								<c:if test="${myselect == 2}">selected="selected"</c:if>>2</option>
							<option value="3"
								<c:if test="${myselect == 3}">selected="selected"</c:if>>3</option>
							<option value="4"
								<c:if test="${myselect == 4}">selected="selected"</c:if>>4</option>
						</select>
					</form>
					<c:if test="${empty myselect}">
						<c:forEach  items="${posts}" var="p" begin="0" end="11"> <!-- ${param.myselect} -->
							<div class="col-md-4">
